    ...
    type ClientProtocolSocket internal (ws: ClientWebSocket) =
        ...
        let receiveLoop (mbox: MailboxProcessor<_>) =
            let inner: Async<Result<unit, _>> =
                asyncResult {
                    // This could have been done using a while loop
                    // instead of recursion.
                    // However, having this as a recursive AsyncResult
                    // means that we can break the loop should
                    // any step fail.
                    let! prevailingCT =
            ...
            do logMessage "Starting receive loop."
