...
module Extensions =
    ...
    [<RequireQualifiedAccess>]
    module List =
        // TODO - Would be nice to have a TCO'd version of this! Possible?
                ...
                async {
                    ...
                    let onException exn =
                        do updateState (function 
                            ...
                            // Although this breaks the contract, not going to be the one to police it here.
                            | State.AwaitingCallbacks _ ->                          
                                state)
                    ...
                    // once the async callbacks are known but have 
                    use _register =
                        ct.Register (fun _ ->
                    ...
                    let awaiter =
                        Async.FromContinuations (fun (onAsyncComplete, onAsyncError, onAsyncCancelled) ->                        
                            do updateState (fun currentState ->
                                // This is to address the situation where we know the async callbacks
                                // but have yet to update our state even though cancellation
                                // has been requested.
    ...
    module Map =
        ...
        let ofAttributeArray (attribs: string []) =
            attribs
            |> Seq.indexed
